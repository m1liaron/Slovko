...
// const filePath = "D:\\Programming\\projects\\Slovko\\backend\\src\\db\\data\\words.json";
...
async function insertWord(entry: Entry) {
    ...
    for (const sense of entry.senses) {
        const senseRes = await client.query(
            `
      INSERT INTO senses (headword_id, pos, definition)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING id
      `,
        ...
        for (const phr of sense.phrases || []) {
            await client.query(
                `INSERT INTO phrases (sense_id, phrase) VALUES ($1, $2)`,
                [senseId, phr]
            );
        }
