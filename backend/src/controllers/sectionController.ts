...
const addSection: AuthRequestHandler = async (req, res) => {
    ...
    const userId = req.user.id;
    try {
        const existGroup = await Section.findOne({
            where: {
                title: data.title,
                userId: userId,
            },
        ...
        const newSection = (await Section.create({ ...data, userId })).toJSON();
...
const updateSection: AuthRequestHandler = async (req, res) => {
    try {
        ...
        if (updatedSection[0] === 0) {
            return res.status(StatusCodes.NOT_FOUND).json({ error: true, message: "Section not found" });
        }
...
const removeSection: AuthRequestHandler = async (req, res) => {
    try {
        ...
        if (!section) {
            res.status(StatusCodes.NOT_FOUND).send({ error: true, message: "Section not found" });
            return;
        }
