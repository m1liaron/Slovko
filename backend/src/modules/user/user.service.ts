...
import { StreakRepository } from "../streak/streak";
...
const UserService = {
    ...
    async updateUserStreak(userId: string) {
        ...
        const streakDates = await StreakRepository.findAllByUserWithSelectedFields(
            userId,
        );
    ...
    async getUserStreakDates(userId: string, month: number, year: number) {
        ...
        return StreakRepository.findAllByUserAndDateRange(userId, startDate, endDate);
    },
